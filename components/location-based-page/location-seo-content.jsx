...
export default function LocationSEOContent({ location }) {
  return (
    <section className="bg-white pt-0 pb-14 md:pb-16 px-3 sm:px-4 md:px-6">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-5 gap-8 lg:gap-12">
        <FadeIn direction="right" className="col-span-3 grid grid-cols-2 md:grid-cols-4 gap-3" duration={0.6} delay={0.2}>
          {[
            ...
            <div
              key={i}
              className={`rounded-[24px] overflow-hidden border border-black/10 bg-white/60 shadow-[0_12px_30px_rgba(26,29,41,0.03)] ${
                i === 2 || i === 3 ? "col-span-2" : ""
              }`}
            >
          <img
